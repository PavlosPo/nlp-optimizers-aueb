# Here we will try to make it for one batch in order to see the corect results
...
dataset = load_dataset('glue', 'cola').map(prepare_dataset, batched=True).select([i for i in range (0,100)])
