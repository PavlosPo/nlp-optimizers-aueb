...
class BertClassifier(nn.Module):
    def __init__(self, num_labels: int, model_name: str = "bert-base-uncased"):
        ...
        self.model = AutoModelForTokenClassification.from_pretrained(self.model_name, num_labels=self.num_labels, return_dict=True)
    ...
    def forward(self, input_ids, attention_mask):
        outputs = self.model(input_ids=input_ids, attention_mask=attention_mask)
        logits = outputs.logits.to( torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu") )
        probabilities = self.softmax(logits.squeeze())
        # print(f"Probabilities in the forward method: {probabilities}")
        # print(f"Probabilities shape in the forward method: {probabilities.shape}")
        return probabilities
