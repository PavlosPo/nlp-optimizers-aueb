...
class CustomTrainer:
    ...
    def train(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        self.functional_model, params, buffers = make_functional_with_buffers(self.original_model)
        ...
        for i in params:
            print(f"Params Before loss fn: {i}")
            break
        ...
        for epoch in range(self.epochs):
            ...
            for i, data in progress_bar:
                ...
                loss = self.loss_fn(params, buffers, input_ids, attention_mask, labels)
                epoch_loss += loss.item()  # Accumulate loss for each batch

                for i in params:
                    print(f"Params after loss fn: {i}")
                    break

                # Calculate gradients based on loss value
                grads = torch.autograd.grad(loss, params, grad_outputs=params)  # Maybe we should remove the tuple ? 
                ...
                loss, probabilities, predictions = self._get_outputs(params, buffers, input_ids, attention_mask, labels)
                epoch_preds.extend(predictions.clone().detach().cpu().numpy())
                epoch_labels.extend(labels.clone().detach().cpu().numpy())
        ...
        return self.functional_model, params, buffers 
    ...
    def loss_fn(self, params: Tuple[Tensor], buffers: Tuple[Tensor], input_ids: Tensor, attention_mask: Tensor, labels: Tensor) -> Tensor:
        ...
        # probs = fmodel(input_ids=input_ids, attention_mask = attention_mask)
        # preds = torch.nn.argmax(probs, dim=1)
        # loss = torch.nn.functional.binary_cross_entropy(preds.squeeze().to(torch.float32), labels.squeeze().to(torch.float32))
        loss , probs , preds = self._get_outputs(params, buffers, input_ids, attention_mask, labels)
        return loss
    
    def _get_outputs(self, params: Tuple[Tensor], buffers: Tuple[Tensor], input_ids: Tensor, attention_mask: Tensor, labels: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        fmodel, _, __ = make_functional_with_buffers(self.original_model)
        probs = fmodel(params, buffers, input_ids=input_ids, attention_mask = attention_mask)
        preds = torch.argmax(probs, dim=1)
        loss = torch.nn.functional.cross_entropy(preds.squeeze().to(torch.float32), labels.squeeze().to(torch.float32))
        return loss, probs, preds

    def validate(self, params, buffers) -> Tuple[float, list, list]:
        ...
        with torch.no_grad():
            for i, data in progress_bar:
                ...
                loss, preds = self.loss_fn(params, buffers, input_ids, attention_mask, labels)
    ...
    def test(self, params, buffers, test_loader: DataLoader):
